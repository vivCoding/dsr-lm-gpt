...
class DSRLMModel(torch.nn.Module):
    ...
    def __init__(self, gpt_model: str) -> None:
        ...
        # temp vars to be used during asyncio
        self.facts_batch = []
        self.ct = 0
    ...
    async def prompt_gpt(self, system_prompt: str, prompt: str, logprobs=True, top_logprobs=1, max_tokens=10):
        if logprobs:
    ...
    async def prompt_for_answer(self, sentences, name1, name2) -> Tuple[str, float]:
        prompt = f"{sentences}\n So {name1} is {name2}'s:"

        completion = await self.prompt_gpt(default_system_prompt, prompt)
    ...
    async def extract_facts(self, sentence_name_pairs):
        facts = defaultdict(int)
        for sentences, names in sentence_name_pairs:
            names = list(names)
            for i in range(len(names)):
                for j in range(i+1, len(names)):
                    answer, prob = await self.prompt_for_answer(sentences, names[i], names[j])
                    ...
                    answer, prob = await self.prompt_for_answer(sentences, names[j], names[i])
                    if answer in relation_id_map:
    ...
    async def make_api_calls(self, sentences):
        contexts = self.get_contexts(sentences)
        facts = await self.extract_facts(contexts)
        self.ct += 1
        print("onto ct", self.ct, end="\r")
        return facts
    
    async def make_api_calls2(self, sentence_batch):
        api_calls = [self.make_api_calls(sentences) for sentences in sentence_batch]
        self.facts_batch = await asyncio.gather(*api_calls)

    def forward(self, X, phase="train"):
        sentence_batch, queries_batch = X

        asyncio.run(self.make_api_calls2(sentence_batch))
        self.ct = 0
        print()
...
class Trainer:
    def __init__(self) -> None:
        self.train_loader, self.val_loader, self.test_loader = clutrr_loader(batch_size=64, max_entries=1000)
